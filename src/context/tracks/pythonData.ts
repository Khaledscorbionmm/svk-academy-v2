export const pythonTrackData = [
  ...
  },
  {
    ...
    "duration_minutes": 15,
    "content": {
      "context": "حان الوقت لتحويل المفاهيم إلى كود! لبناء كائن خاص بك في بايثون، يجب أن تبدأ بتعريف الفئة الخاصة به، تماماً كما يتم تعريف أنواع البيانات الأساسية (مثل القوائم أو النصوص) كفئات في خلفية بايثون.",
      "description": "نتعلم هنا التطبيق العملي لبناء الفئات (Classes). سنقوم باستخدام الكلمة المفتاحية 'class' لإنشاء فئة، وسنتعلم الدالة السحرية '__init__' التي تمثل 'المُنشئ' (Constructor) والتي تُنفذ تلقائياً عند صنع كائن جديد. كما سنفهم دور الكلمة 'self' لربط البيانات بالكائن نفسه.",
      "prototype": "class ClassName:\n    def __init__(self, arg1):\n        self.attribute1 = arg1\n\n    def method_name(self):\n        print(self.attribute1)",
      "parameters": "self: يمثل الكائن الحالي المُنْشأ من الفئة، وهو إلزامي كأول معامل في أي طريقة (Method). __init__: هي دالة التهيئة المبدئية.",
      "return_value": "استدعاء اسم الفئة كدالة (مثال: ClassName()) يرجع كائناً جديداً (Object/Instance) من تلك الفئة.",
      "example": "class Dog:\n    def __init__(self, name):\n        self.name = name\n    def bark(self):\n        print(f'{self.name} يقول هو هو!')\n\nmy_dog = Dog('ريكس')\nmy_dog.bark()"
    },
    ...
    "quick_practical_examples": [
      {
        "type": "correct",
        "title": "إنشاء عدة كائنات مختلفة",
        "code": "class Player:\n    def __init__(self, hero_name):\n        self.name = hero_name\n        self.level = 1\n\np1 = Player('صقر')\np2 = Player('ظل')\nprint(f'{p1.name} ضد {p2.name}')",
        "expected_output": "صقر ضد ظل",
        "explanation": "أنشأنا فئة واحدة، ولكن استخرجنا منها كائنين مستقلين تماماً (p1 و p2)، كل كائن يحتفظ ببياناته الخاصة بواسطة المتغير `self`."
      },
      {
        "type": "wrong",
        "title": "نسيان كلمة self في دوال الفئة",
        "code": "class Car:\n    def start_engine():\n        print('المحرك يعمل')\n\nmy_car = Car()\nmy_car.start_engine()",
        "error_message": "TypeError: start_engine() takes 0 positional arguments but 1 was given",
        "explanation": "عند استدعاء أي دالة من كائن، تقوم بايثون بتمرير الكائن نفسه تلقائياً كأول معامل للدالة. لذلك يجب أن نكتب `def start_engine(self):` لتستقبل الدالة هذا الكائن حتى وإن لم نستخدمه."
      },
      {
        "type": "mistake",
        "title": "إعطاء قيم ثابتة في الفئة بدلاً من __init__",
        "explanation": "من الأخطاء كتابة متغيرات مباشرة داخل الفئة (Class Variables) بقصد إعطائها للكائنات. هذا يجعل المتغير مشتركاً بين جميع الكائنات! لتحديد خصائص مستقلة لكل كائن، يجب تعريفها داخل الدالة السحرية `__init__` باستخدام `self`."
      },
      {
        "type": "use_case",
        "title": "نماذج قواعد البيانات (Models)",
        "explanation": "في أطر عمل الويب مثل Django، يتم تمثيل جداول قاعدة البيانات باستخدام Classes. جدول المستخدمين هو `class User`، وكل صف (مستخدم) في قاعدة البيانات يمثل Object من هذا الـ Class."
      },
      {
        "type": "challenge",
        "title": "تحدي: تعديل الخصائص",
        "code": "class Product:\n    def __init__(self, name, price):\n        self.name = name\n        self.price = price\n\nitem = Product('حاسوب', 1000)\n# اكتب الكود لزيادة السعر بمقدار 200\n________\nprint(item.price)",
        "expected_output": "1200",
        "explanation": "يمكنك الوصول إلى خصائص الكائن وتعديلها من الخارج باستخدام النقطة (Dot Notation) عن طريق كتابة `item.price += 200`."
      }
    ],
    "lesson_type": "milestone",
    "exam_data": {
      "title": "اختبار الفئات والكائنات",
      "questions": [
        {
          "question": "ما هي الوظيفة الأساسية للدالة السحرية `__init__` في بايثون؟",
          "options": [
            "حذف الكائن من الذاكرة",
            "تهيئة الكائن الجديد وإعداد قيمه المبدئية عند إنشائه",
            "طباعة بيانات الكائن على الشاشة",
            "تحويل الكائن إلى نص"
          ],
          "correct_answer": "تهيئة الكائن الجديد وإعداد قيمه المبدئية عند إنشائه",
          "explanation": "الدالة `__init__` تعمل كمُنشئ (Constructor) وتستدعى أوتوماتيكياً في اللحظة التي يتم فيها إنشاء نسخة جديدة (Object) من الفئة."
        }
      ]
    }
  },
